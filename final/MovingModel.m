function ME = MovingModel(ME, maxSpeed, acc, dec, MRT_s)
    if ME.status == -2
    ...
    distToDestination = sqrt((MRT_s{1, ME.id_station_end}.pos_x - ME.pos_x)^2 + (MRT_s{1, ME.id_station_end}.pos_y - ME.pos_y)^2);
    if distToDestination <= ME.velocity
        ME.pos_x = MRT_s{1, ME.id_station_end}.pos_x;
        ME.pos_y = MRT_s{1, ME.id_station_end}.pos_y;
        ME.status = -2;
        ME.velocity = 0;
